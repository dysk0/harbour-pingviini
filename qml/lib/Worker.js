...
WorkerScript.onMessage = function(msg) {
        ...
        cb.__call(
                    ...
                    function (reply) {
                        ...
                        for (i; i < length; i++) {
                            ...
                            if (msg.mode === "append") {
                                tweet = parseTweet(reply.statuses[i], msg.modelUsers)
                                //fillModelUsers(msg.modelUsers, )
    ...
    if (msg.bgAction){
        ...
        cb.__call(msg.bgAction, msg.params, function (reply, rate, err) {
            ...
            if (msg.model){
                var tweets = [];
                if (msg.bgAction === "search_tweets" && 'statuses' in reply){
                    tweets = reply.statuses;
                } else if (msg.bgAction === "directMessages_events_list" && 'events' in reply){
                    //parseDM.append(reply.events)
                    //msg.model.append(parseDM.getList());
                } else {
                    tweets = reply;
                }
                ...
                for(i; i < length; i++){
                    ...
                    if (msg.bgAction === "search_tweets"){
                        ...
                        if (msg.mode === "append") {
                            tweet = parseTweet(tweets[i], msg.modelUsers);
                            msg.model.append(tweet)

                        }
                        if (msg.mode === "prepend") {
                            tweet = parseTweet(tweets[length-i-1], msg.modelUsers);
